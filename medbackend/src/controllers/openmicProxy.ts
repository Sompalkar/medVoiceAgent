...
export async function createBot(req: Request, res: Response) {
  try {
    ...
    console.error("[openmic] createBot error status:", status, "detail:", detail);
...
export async function updateBot(req: Request, res: Response) {
  try {
    ...
    if (incoming.instructions && !incoming.prompt) patch.prompt = incoming.instructions;
    if (incoming.prompt && !incoming.instructions) patch.instructions = incoming.prompt;
